...
def _preprocess_bbox(bbox_mode, canvas_size, examples, is_train=True,
    ...
    ret_bboxes_view = torch.zeros(B, N_out, max_len, *bbox_shape)
...
def collate_fn(
    ...
    lidar2imgs =  torch.stack([example['lidar2image'].data for example in examples], dim=0)

    if 'img_aug_matrix' in examples[0]:
        img_aug_matrix =  torch.stack([example['img_aug_matrix'].data for example in examples], dim=0)
        lidar2imgs = torch.matmul(img_aug_matrix, lidar2imgs) 
...
def lidar_collate_fn(
    ...
    # mask
    # keys = []
    # if "gt_masks_bev" in examples[0] and examples[0]["gt_masks_bev"] is not None:
    #     keys += ["gt_masks_bev"]
    # if "gt_aux_bev" in examples[0] and examples[0]["gt_aux_bev"] is not None:
    #     keys += ["gt_aux_bev"]
    #     assert bbox_drop_ratio == 0, "map is not affected in bbox_drop"
    ...
    #         example[key] for key in keys  # np array, channel-last
    #     ], axis=0)).float() for example in examples], dim=0)  # float32
    #     ret_dict.update({"bev_map_with_aux": bev_map_with_aux})
    # fmt: on
    ...
    if "range_img" in examples[0]:
        range_img = torch.stack(
            [example["range_img"].data for example in examples])      
        range_img = range_img.to(
            memory_format=torch.contiguous_format).float()
        range_img = range_img.permute(0, 3, 1, 2)
        ret_dict["range_img"] = range_img

    if "range_mask" in examples[0]:
        range_mask = torch.stack(
            [example["range_mask"].data for example in examples])      
        range_mask = range_mask.to(
            memory_format=torch.contiguous_format).float()
        ret_dict["range_mask"] = range_mask
...
def multi_bbox_collate_fn(
    ...
    if bbox_mode is not None:
        # NOTE: both can be None
        img_bboxes_3d_input, img_bbox_view_coord = _preprocess_bbox(
            bbox_mode, canvas_size, examples, is_train=is_train,
            view_shared=bbox_view_shared, bbox_add_ratio=bbox_add_ratio,
            bbox_add_num=bbox_add_num, bbox_drop_ratio=bbox_drop_ratio)
        pc_bboxes_3d_input, _ = _preprocess_bbox(
            bbox_mode, canvas_size, examples, is_train=is_train,
            view_shared=True, bbox_add_ratio=bbox_add_ratio,
            bbox_add_num=bbox_add_num, bbox_drop_ratio=bbox_drop_ratio)
        ret_dict["kwargs"]["bboxes_3d_data_pc"] = pc_bboxes_3d_input
        ret_dict["kwargs"]["bboxes_3d_data_img"] = img_bboxes_3d_input
        ret_dict["kwargs"]["bboxes_view_data_coords_img"] = img_bbox_view_coord
...
def cylinder_bbox_collate_fn(
    ...
    stacked_points = [example['points'].data for example in examples]

    ret_dict['points'] = DC(stacked_points, examples[0]['points'].stack, examples[0]['points'].padding_value)
