...
class PCValidatorBox:
    def __init__(self, cfg, val_dataset, pipe_cls, pipe_param) -> None:
        ...
        logging.info(
            f"[PCValidatorBox] Validator use model_param: {pipe_param.keys()}")
    ...
    def validate(
        ...
        logging.info("[PCValidatorBox] Running validation... ")
        bbox_embedder.eval()  # important !!!
        unet.eval()
        ...
        for validation_i in self.cfg.runner.validation_index:
            ...
            val_input = lidar_bbox_collate_fn(
            ...
            for _ in range(self.cfg.runner.validation_times):
                ...
                gen_img = unnormalize_range_img(gen_img, mean, std, to_01=True, log_scale=log_scale)
                range_img = unnormalize_range_img(range_img, mean, std, to_01=True, log_scale=log_scale)
