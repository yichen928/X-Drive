...
from .blocks_multiview_condition import (
    BasicMultiViewConditionTransformerBlock,
)


class UNet2DSepConditionModelMultiview(UNet2DConditionModel):
    r"""
    ...
    def __init__(
        ...
        cross_view_type: str = "default",
        img_pos_embedding: bool = False,
        num_freqs: int = 4,
        separate_condition: bool = True,
        use_gsa: bool = False,
    ):
        super().__init__(
        ...
        for name, mod in list(self.named_modules()):
            if isinstance(mod, BasicTransformerBlock):
                if crossview_attn_type == "basic":
                    _set_module(self, name, BasicMultiViewConditionTransformerBlock(
                        **mod._args,
                        ...
                        img_size=img_size,
                        img_pos_embedding=img_pos_embedding,
                        num_freqs=num_freqs,
                        separate_condition=separate_condition,
                        use_gsa=use_gsa
                    ))
                else:
    ...
    def forward(
        ...
        lidar2imgs: Optional[torch.Tensor] = None,
